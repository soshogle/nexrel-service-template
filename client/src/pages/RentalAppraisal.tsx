...
export default function RentalAppraisal() {
  ...
  const submit = trpc.inquiries.submit.useMutation({
    onSuccess: () => {
      toast.success("Your rental appraisal request has been received. We'll be in touch shortly.");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    submit.mutate({
      ...
      message: `Rental Appraisal Request\nAddress: ${formData.address || "Not provided"}\n\n${formData.message || ""}`,
    });
  };

  return (
    <div className="pt-20">
      <PageHero label="RENTAL APPRAISAL" title="Free Rental Appraisal" subtitle="Find out what your investment property could earn in rent. No obligation." />
        ...
        <div className="container max-w-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <Input value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="Property address" className="border-[#214359]/20 h-12" />
            </div>
            <div>
            ...
            <Button type="submit" disabled={submit.isPending} className="w-full bg-[#86C0C7] hover:bg-[#6AABB3] text-white h-12 uppercase tracking-wider">
              {submit.isPending ? "Submitting..." : "Request Free Rental Appraisal"}
            </Button>
          </form>
        </div>
      </section>
