...
export default function Renting() {
  ...
  const services = [
    { icon: Home, title: safeStr(pageLabels?.forLease ?? t("common.forLease")), description: String(t("renting.forLeaseDesc")), href: "/for-lease" },
  ];

  return (
    <div className="pt-20">
      <PageHero label={safeStr(pageLabels?.renting ?? t("nav.renting")).toUpperCase()} title={String(t("renting.title"))} subtitle={String(t("renting.subtitle"))} />
            ...
            {services.map((s) => (
              <Link key={s.href} href={s.href} className="group block p-8 border border-[#E8F4F4] rounded-sm hover:border-[#86C0C7] hover:shadow-lg transition-all">
                ...
                <span className="inline-flex items-center gap-2 text-[#86C0C7] font-medium tracking-wider uppercase text-sm group-hover:gap-3 transition-all">
                  {String(t("common.learnMore"))} <ArrowRight size={14} />
                </span>
              </Link>
            ))}
          </div>
